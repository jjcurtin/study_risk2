...
test_metrics_main <- read_csv(here::here(path_models, 
                                    "best_config_xgboost_v7_kfold_dyn_dem_oud.csv"), 
                              show_col_types = FALSE) |> 
  select(split_num, "main" = roc_auc) |> 
  arrange(split_num)

test_metrics_baseline <- read_csv(here::here(path_models, 
                                    "best_config_xgboost_v1_kfold_baseline.csv"), 
                              show_col_types = FALSE) |> 
...
pp_perf_tibble |> 
  write_csv(here::here(path_models, "pp_perf_tibble_xgboost_v7.csv"))

pp_tidy |> 
  write_csv(here::here(path_models, "posteriors_xgboost_v7.csv"))

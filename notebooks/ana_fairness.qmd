...
auroc_all <- read_csv(here::here(path_models, "fairness_auroc_6_x_5_1_x_5_nested_v13_full.csv"),
...
calc_pp <- function (data, dem_var) {
  ...
  } else if (dem_var == "orientation") {
    data |> 
    select(id = repeat_num, id2 = fold_num, `heterosexual`, `not heterosexual`)
  } else {
  ...
  contrast_lists <- 
    ...
    c(list("not heterosexual"), list("heterosexual"))
  } else {
    stop(dem_var, " not in data")
  }
...
orientation <- calc_pp(auroc_all, "orientation")
geography <- calc_pp(auroc_all, "geography")
```

Bind all pp/contrast tibbles and save
```{r}
list(gender[[1]], income[[1]], race[[1]], orientation[[1]], geography[[1]]) |> 
  bind_rows() |> 
  write_csv(here::here(path_models, "posteriors_fairness_v13_full.csv"))

list(gender[[2]], income[[2]], race[[2]], orientation[[2]], geography[[2]]) |> 
  bind_rows() |> 
  write_csv(here::here(path_models, "pp_fairness_v13_full.csv"))

list(gender[[3]], income[[3]], race[[3]], orientation[[3]], geography[[3]]) |> 
  bind_rows() |>
  write_csv(here::here(path_models, "pp_fairness_contrast_v13_full.csv"))
```

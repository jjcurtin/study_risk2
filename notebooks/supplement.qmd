...
suppressMessages(library(kableExtra, exclude = "group_rows"))
...
shaps <- read_rds(here::here(path_models, "shapsgrp_1_x_5_day_v10_kfold_full_final_raw.rds")) |> 
  mutate(variable_grp = replace_values(variable_grp,
                                       "gender_male" ~ "Male (demographic)",
                                       "gender_not.male" ~ "Not male (demographic)",
                                       "label_day_Mon" ~ "Monday (label)",
                                       "label_day_Tue" ~ "Tuesday (label)",
                                       "label_day_Wed" ~ "Wednesday (label)",
                                       "label_day_Thu" ~ "Thursday (label)",
                                       "label_day_Fri" ~ "Friday (label)",
                                       "label_day_Sat" ~ "Saturday (label)",
                                       "label_day_Sun" ~ "Sunday (label)",
                                       "employment_employed" ~ "Employed (demographic)",
                                       "employment_not.employed.looking" ~ "Employed, looking (demographic)",
                                       "employment_not.employed.not.looking" ~ "Employed, not looking (demographic)",
                                       "orientation_heterosexual" ~ "Heterosexual (demographic)",
                                       "orientation_not.heterosexual" ~ "Not heterosexual (demographic)",
                                       "race_Hispanic.and.or.not.white" ~ "Hispanic and/or not White (demographic)",
                                       "race_non.hispanic.white" ~ "Non-Hispanic White (demographic)"))

preds_fairness <- read_rds(here::here(path_models, "fairness_preds_1_x_5_day_v10_kfold_full_final.rds"))

pp_perf_tibble_shap_fairness <- read_csv(here::here(path_models, "pp_perf_tibble_shap_fairness.csv"),
                                         show_col_types = FALSE)

perf_month <- read_csv(here::here(path_models, "test_auroc_month_in_recovery.csv"),
                       show_col_types = FALSE)
```
...
### Figure S1: Model performance by month into recovery
...
perf_month |> 
  ...
  geom_point() +
  geom_line() +
  labs(x = "Month in recovery (binned in 3-month intervals)") +
  theme_classic() +
...
### Figure S2: Feature Importance for all Feature Categories
...
dem_vars <- c("Age (demographic)", "Geography (demographic)", "Income (demographic)", 
              "Education (demographic)")
one_hot_vars <- c("Male (demographic)",
                  "Not male (demographic)",
                  "Monday (label)",
                  "Tuesday (label)",
                  "Wednesday (label)",
                  "Thursday (label)",
                  ...
                  "Employed, looking (demographic)",
                  "Employed, not looking (demographic)",
                  "Heterosexual (demographic)",
                  "Not heterosexual (demographic)",
                  "Hispanic and/or not White (demographic)",
                  "Non-Hispanic White (demographic)")
...
shaps|>
  ...
  geom_point(alpha = .3) +
  # geom_smooth(data = ~ dplyr::filter(.x, !variable_grp %in% c(dem_vars, one_hot_vars, "Cannabis use, Raw (EMA)")),
  #             method = "gam",
  #             formula = y ~ s(x, k = 30),

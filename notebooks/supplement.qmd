...
### Figure S1: Calibration Plots by Fairness Group

Calibration by fairness group
```{r}
bin_width <- 0.1

preds_all_logi <- preds_fairness |> 
  mutate(bins = cut(prob_logi, breaks = seq(0, 1, bin_width)), 
         lapse = if_else(label == "Lapse", 1, 0),
         prob = "Platt calibrated")  

preds_all_raw <- preds_fairness |> 
  mutate(bins = cut(prob_raw, breaks = seq(0, 1, bin_width)), 
         lapse = if_else(label == "Lapse", 1, 0),
         prob = "Raw (uncalibrated)") 

preds_plot <- preds_all_raw |> 
  bind_rows(preds_all_logi) |> 
  mutate(prob = factor(prob, levels = c("Raw (uncalibrated)", 
                                        "Platt calibrated"))) |> 
  pivot_longer(geography:orientation, names_to = "group", values_to = "subgroup") |> 
  mutate(group = factor(group,
                        levels = c("gender", "race", "income", "orientation", "geography"),
                        labels = c("Gender", "Race", "Income", "Orientation", "Geography")),
         subgroup = factor(subgroup, levels = c("not male", "male", 
                                            "Hispanic and/or not white", "non-Hispanic White", 
                                            "below poverty", "above poverty", 
                                            "not heterosexual", "heterosexual",
                           ...
                           labels = c("Not male", "Male", 
                                            ...
                                            "Not heterosexual", "Heterosexual",
                                            "Small town/rural", "Urban/suburban"))) |>
  group_by(bins, group, subgroup, prob)  |> 
  summarize(mean_lapse = mean(lapse, na.rm = TRUE),
            n = sum(!is.na(lapse)),
            .groups = "drop") |> 
  filter(n >= 10) |> 
  select(-n) |> 
...
orientation <- preds_plot |> 
  filter(group == "Orientation") |> 
  ggplot(aes(x = midpoints, y = mean_lapse, group = prob,
             color = prob, linetype = prob)) +
  facet_grid(group~subgroup) +
  geom_abline(slope = 1, intercept = 0, linetype = "longdash", color = "gray80") +
...
```{r}
#| fig-height: 10
gender/race/income/orientation/geography
```

### Figure S2: Global Feature Importance by Fairness Group

```{r}
#| code-fold: true

shaps_fairness <- shaps |>
  right_join(preds_fairness, by = "id_obs")
...
global_race <- shaps_fairness |>
  group_by(race, variable_grp) |>
  summarize(mean_value = mean(abs(value)), .groups = "drop") |> 
  ...
  labs(y = "Mean(|Shapley Value|)",
       x = NULL,
       fill = NULL) +
  scale_fill_manual(values = c("#046B52", "#240e31")) +
  theme_classic() +
  theme(axis.text=element_text(size=9.5),
        legend.key.size = unit(0.25, "cm"),
        panel.border = element_rect(colour = "black", fill = NA, linewidth = 1),
...
n_obs <- shaps_fairness |> 
  group_by(race) |> 
  summarize(n_obs = n(), .groups = "drop")

shaps_day_max <- shaps_fairness |>
  group_by(id_obs) |>
  slice_max(value) |> 
  left_join(n_obs, by = "race") |> 
  select(variable_grp, value, race, n_obs) |> 
  group_by(race, variable_grp) |> 
  summarise(
  n = n(),
  prop = n / first(n_obs)) |> 
...
global_race / local_race
```

```{r}
#| code-fold: true

global_gender <- shaps_fairness |>
  group_by(gender, variable_grp) |>
  summarize(mean_value = mean(abs(value)), .groups = "drop") |> 
   filter(variable_grp %in% shap_levels) |> 
   mutate(variable_grp = factor(variable_grp, levels = shap_levels),
         gender = factor(gender, levels = c("not male", "male"))) |> 
  ggplot(mapping = aes(x = variable_grp, y = mean_value)) +
...
shaps_day_max <- shaps_fairness |>
  group_by(id_obs) |>
  slice_max(value) |> 
  left_join(n_obs, by = "gender") |> 
  select(variable_grp, value, gender, n_obs) |> 
  group_by(gender, variable_grp) |> 
  summarise(
  n = n(),
  prop = n / first(n_obs)) |> 
...
global_gender / local_gender
```



### Figure S3: Global Feature Importance for all Feature Categories
...
### Figure S4: Individual Shapley Plots for all Feature Categories

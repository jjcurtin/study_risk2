...
shaps_out_grp <- shaps_out |>
  ...
  mutate(variable_grp = if_else(str_detect(variable, ".lapse") & 
         ...
         variable_grp = if_else(str_detect(variable, "med_"),
                                "MOUD efficacy (baseline)",
                                variable_grp),
         variable_grp = if_else(str_detect(variable, "od_hx"),
                                "Overdose history (baseline)",
                                variable_grp),
         variable_grp = if_else(str_detect(variable, "preferred_opioid"),
                                "Preferred opioid (baseline)",
                                variable_grp),
         variable_grp = if_else(str_detect(variable, "preferred_roa"),
                                "Preferred ROA (baseline)",
                                variable_grp)) |> 
  # rename other baseline features
  mutate(variable_grp = case_match(variable_grp,
                                  "counseling_freq" ~ "Counseling frequency (baseline)",
                                  "self_help_meetings_freq" ~ "Self-help meeting frequency (baseline)",
                                  "dsm_c"  ~ "DSM-5 OUD symptom count (baseline)",
                                  "past_month_opioid_use_yes" ~ "Past month opioid use (baseline)",
                                  "past_month_detox_or_residential_Yes" ~ "Past month residential treatment (baseline)",
                                  "past_month_psych_med_yes" ~ "Past month psychiatric medication (baseline)",
                                  "recovery_1_satisfaction" ~ "Recovery satisfaction (baseline)",
                                  "recovery_2_motivation" ~ "Recovery motivation (baseline)",
                                  "recovery_3_confidence" ~ "Recovery confidence (baseline)",
                                  "recovery_4_avoid_other_drugs_Yes" ~ "Avoiding other drugs (baseline)",

...
results_all %>% 
  ...
  write_csv(here::here(path_models, str_c("inner_metrics_1_x_5_day", 
                                           "_", version, "_", 
                                            cv, "_", model, ".csv")))
...
metrics_avg |>
  group_by(outer_split_num)
  select(outer_split_num, hp1)
```

### Plot hyperparameters

Plot HPs by `outer_split_num`.
```{r}
dhp4 <- results_all |> 
  group_by(outer_split_num, hp1, hp2, hp3, hp4) |>  
   summarize(roc_auc = median(roc_auc),
...
plot(x = dhp4$hp4, y = dhp4$roc_auc)
```

...
shaps_out_grp <- shaps_out |>
  ...
  mutate(variable_grp = if_else(str_detect(variable, ".lapse") & 
         ...
         variable_grp = if_else(str_detect(variable_grp, "label_day"), 
                                "Day (label)",
                                variable_grp),
         variable_grp = if_else(str_detect(variable, "med_"),
                                "MOUD efficacy, Raw (baseline)",
                                variable_grp),
         variable_grp = if_else(str_detect(variable, "od_hx"),
                                "Overdose history, Raw (baseline)",
                                variable_grp),
         variable_grp = if_else(str_detect(variable, "preferred_opioid"),
                                "Preferred opioid, Raw (baseline)",
                                variable_grp),
         variable_grp = if_else(str_detect(variable, "preferred_roa"),
                                "Preferred ROA, Raw (baseline)",
                                variable_grp)) |> 
  # rename other baseline features
  mutate(variable_grp = case_match(variable_grp,
                                  "counseling_freq" ~ "Counseling frequency, Raw (baseline)",
                                  "self_help_meetings_freq" ~ "Self-help meeting frequency, Raw (baseline)",
                                  "dsm_c"  ~ "DSM-5 OUD symptom count, Raw (baseline)",
                                  "past_month_opioid_use_yes" ~ "Past month opioid use, Raw (baseline)",
                                  "past_month_detox_or_residential_Yes" ~ "Past month residential treatment, Raw (baseline)",
                                  "past_month_psych_med_yes" ~ "Past month psychiatric medication, Raw (baseline)",
                                  "recovery_1_satisfaction" ~ "Recovery satisfaction, Raw (baseline)",
                                  "recovery_2_motivation" ~ "Recovery motivation, Raw (baseline)",
                                  "recovery_3_confidence" ~ "Recovery confidence, Raw (baseline)",
                                  "recovery_4_avoid_other_drugs_Yes" ~ "Avoiding other drugs, Raw (baseline)",
                                  
                                  .default = variable_grp),
         variable_grp = factor(variable_grp)) |> 

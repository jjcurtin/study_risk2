...
```{r}
preds <- read_rds(here::here(path_models, "outer_preds_6_x_5_1_x_5_day_v13_nested_full.rds"))
...
auroc_dem <- get_auroc_by_dem(df = feat_preds, var = "gender") |> 
  left_join(get_auroc_by_dem(df = feat_preds, var = "race"), by = "outer_split_num") |> 
  left_join(get_auroc_by_dem(df = feat_preds, var = "income"), by = "outer_split_num") |> 
  left_join(get_auroc_by_dem(df = feat_preds, var = "orientation"), by = "outer_split_num")  |> 
  left_join(get_auroc_by_dem(df = feat_preds, var = "geography"), by = "outer_split_num")  
```
...
```{r}
auroc_dem |> 
  write_csv(here::here(path_models, "fairness_auroc_6_x_5_1_x_5_nested_v13_full.csv"))  
```

...
### Feature Engineering
Features were engineered from two types of data sources: 

1. Static individual differences. 

2. Dynamic sensing data.
...
2. Resampling of minority outcome
...
## Participants
Data were collected from April 2021 through December 2024. A total of 336 participants were eligible, consented, and remained on study for at least one month. Our final analysis sample consisted of 299 participants. Ten participants were excluded due to unusually low adherence^[One participant completed only 3 EMA prompts over 88 days and 11 participants had fewer than 20 geolocation points per day on average.], 13 participants were excluded due to insufficient context data for geolocation points^[We required participants have at least two contextualized locations other than their home.], 4 participants were excluded due to frequent lapses suggesting they no longer had a goal of abstinence, 8 participants were excluded due to careless responding. A summary of participant demographic and OUD characteristics is presented in @tbl-1.

{{< embed notebooks/mak_tables.qmd#tbl-1 >}}

## Adherence, Features, and Labels
Median days on study across participants was 356 days (range 32-395 days). 83% of participants remained on study for at least six months. EMA adherence was high. On average participants completed 73% of the daily EMA prompts (range 24-100%). Participants provided on average 311 geolocation points daily (range 20-825). 

Our final model consisted of 640 features. The proportion of missing values across features was low (mean=.03, range = 0-.12). Across participants we generated a total of 89,125 labels. Thirty-nine percent of participants reported an opioid lapse while on study (mean=5.34, range 0-76). This resulted in 1.80% of the labels positive for lapse (1,608/89,125 labels). 


## Performance
The best model configuration used an xgboost statistical algorithm and up-sampled the minority class.^[The best model configuration used 1:4 upsampling of the minority class and the following hyperparameter values: learning rate = .1, tree depth = 1, mtry = 50]<!--added this footnote to follow TRIPOD-AI checklist--> The median posterior probability for the best model was `r round(pp_perf$pp_median[1], 2)`, with narrow 95% CI ([`r round(pp_perf$pp_lower[1], 2)`, `r round(pp_perf$pp_upper[1], 2)`]). 
...
# Discussion
<!--Discussion notes:
- don’t have people have over 65 important group to look at but not in our sample -->

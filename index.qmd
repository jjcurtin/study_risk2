...
### Feature Engineering
Features were calculated using only data collected before the start of each prediction window to ensure our models were making true future predictions. We calculated a total of 793<!--update--> features from three data sources: 
...
We initially considered five statistical algorithms that differed in terms of assumptions and bias-variance tradeoff: elastic net, random forest, XGBoost, a single-layer neural network, and a support vector machine. Preliminary analyses using simple 6x5 cross-validation suggested that XGBoost was superior (see supplement for performance estimates by algorithm). Additionally, we planned to use the Shapley additive explanations (SHAP) method for calculating feature importance which is optimized for XGBoost. Therefore we only considered XGBoost for the final nested cross-validation analyses.
...
### Model Evaluation
We evaluate the best full model’s probability predictions across three domains: discrimination, calibration, and overall performance. We follow recommendations for reporting measures and plots to characterize these performance domains [@vancalsterEvaluationPerformanceMeasures2025]. 
...
## Participants
We enrolled 330 participants across 47 states in the United States from April 2021 through December 2024. To be considered enrolled in the study, participants were required to be eligible, consent, and complete the first personalized monthly survey where geolocation context were provided.^[405 participants consented to participate in the study and provided at least one EMA; however, 75 participants dropped out prior to completing the first personalized monthly survey and were therefore not considered fully enrolled in the study.] Of these 330, 14 were withdrawn prior to providing at least 28 days of EMA data due to low EMA adherence^[Participants were instructed that they would be withdrawn for completing fewer than 20 daily surveys in a 28-day period. We applied a more conservative criterion and withdrew participants when adherence fell below 33% (roughly 9 surveys in a 28-day period).<!--not sure how specific to be regarding prospective 28 day adherence and the longitudinal considerations-->], 7 participants were excluded due to evidence of careless responding on daily surveys, and 13 participants were excluded due to having insufficient context data for geolocation points (fewer than two contextualized locations). Our final analysis sample consisted of 296 participants. Participant demographic and OUD characteristics is presented in @tbl-1.
...
Our final feature set consisted of 674 features.<!--update--> The proportion of missing values across features was low (mean=2.7%, range = 0-12.4%).<!--update--> Across participants we generated a total of 82,156 day-level labels. Training sets (N=30) had, on average, 70,886 labels (range 69,025-73,129) from 239 participants (range 239-240) <!--JJC: this is training sets from the outer loop? KW: needs to update-->. Thirty-six percent of participants (106/296) reported an opioid lapse while on study (mean=4.93, range 0-117). This resulted in 1.8% of the labels positive for lapse (1,458/82,156 labels). We stratified the data on a variable of whether someone lapsed on study to ensure our imbalanced outcome was evenly split over folds (mean=.018, range =.016-.020)<!--JJC: you move back and forth between percentages and proportions.  Stick with one. I think percentages are clearer because easy to report the units %--><!--KW update with final splits-->
...
<!--update to table for 4 model contrasts between full and ablated models-->

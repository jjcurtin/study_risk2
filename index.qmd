...
2. *Demographic individual differences.* For demographics, we created ordinal features for age, education, and income based on predefined response ranges (see @tbl-1 for ranges), an ordinal feature for RUCA code associated with home address (range 1-10), and dummy coded features for gender (male vs. not male) and race and ethnicity (non-Hispanic White vs. Hispanic and/or not White).  
